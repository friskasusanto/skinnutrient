...
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            ...
            <a type="button" class="btn btn btn-secondary btn-rounded btn-rounded pull-right" data-toggle="modal" data-target="#addModal"><i class="fa fa-plus"></i> Tambah Distributor</a>
        </div>
        <div class="card-body">
            @if (count($errors) > 0)
                ...
                <div class="table-responsive">
                    <table id="zero_config" class="table table-striped table-bordered no-wrap">
                        ...
                        @if(count($distributor) != 0)
                            @foreach ($distributor as $key =>$u)
                            <tr>
                                <td>{{++$key}}</td>
                                <td>{{$u->name}}</td>
                                <td>{{$u->alamat}}</td>
                            @if ($u->status == 1)
                                <td><p style="color: green ">distributor aktif</p></td>
                            @elseif ($u->status == 0)
                                <td><p style="color: red ">distributor tidak aktif</p></td>
                            @else
                                <td></td>
                            @endif
                                <td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><center>Tambah Distributor</center></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form novalidate="novalidate" method="POST" action= "{{url('/admin/addDistributor')}}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                        <div class="form-body">
                            <div class="row">
                                <div class="col-md-3">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label>Kontak Distributor</label>
                                    </div>
                                </div>
                                <div class="col-md-9">
                                    <div class="form-group">
...
<div class="modal" id="modalDetail{{$u->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><center>Data Distributor</center></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <tr>
                    <th>Status</th>
                @if ($u->status == 1)
                    <td>: Distributor Aktif</td>
                @else
                    <td>: Distributor Tidak Aktif</td>
                @endif
                </tr>
            </div>
            <div class="modal-footer">
                            ...
                            <div class="col-md-9">
                                <div class="form-group">
                                    <select name="status" type="text" class="form-control pull-right select2" style="width: 100%">
                                          <option value="">--pilih status--</option>
                                          @if ($u->status == 1)
                                          <option value="">--distributor aktif--</option>
                                          @else
                                          <option value="">--distributor tidak aktif--</option>
                                          @endif
                                          <option value= "1">distributor aktif</option>
                                          <option value= "0">distributor tidak aktif</option>
                                    </select>
